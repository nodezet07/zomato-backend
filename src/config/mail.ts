...
async function sendViaBrevoApi(
  ...
  try {
    ...
    let senderName = "Food App";
    let senderEmail = config.SMTP_USER || "antigravityfree70@gmail.com";
    ...
    if (!res.ok) {
      const body = await res.text();
      logger.error(`Brevo API failed to ${to}: ${res.status} ${body.slice(0, 200)}`);
      return false;
    }

    const data = (await res.json()) as { messageId?: string };
    logger.info(`Brevo API email sent to ${to}: ${data.messageId ?? "ok"}`);
    return true;
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    logger.error(`Brevo API failed to ${to}: ${message}`);
    return false;
  }
}
...
/** Resend API first (works on Render), then Brevo API, then SMTP fallback. */
export async function trySendEmail(
  to: string,
  subject: string,
  html: string,
  ...
  if (!isProd) {
    logger.warn(`[DEV] Email skipped (no Resend/Brevo/SMTP): ${to} — ${subject}`);
  }
  return false;
}
